...
$(BINDIR)/$(BINNAME): $(SRC)
	@echo
	@echo  "==> Building ./cmd/rq-hub $(BINDIR)/$(BINNAME) <=="
	GO111MODULE=on go build $(GOFLAGS) -ldflags '$(LDFLAGS)' -o $(BINDIR)/$(BINNAME) ./cmd/rq-hub
...
.PHONY: install
install:
	GO111MODULE=on go build -i $(GOFLAGS) -ldflags '$(LDFLAGS)' -o $(INSTALLPATH)/$(BINNAME) ./cmd/rq-hub
